...
export default function PatientDashboardPage() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Fallback local persistence so user sees updates immediately
      const updatedCustom: CustomProfileData = {
        address: editAddress.trim() || "New Delhi, Delhi",
        date_of_birth: editDob.trim() || "15 Jan 2004",
        gender: editGender,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC] text-[#1E293B]">
      ...
      <main className="flex-1 max-w-[1240px] w-full mx-auto px-4 py-8 md:px-8 space-y-6">
        ...
        {/* ── 1. Top Profile Hero Section (Enhanced Home Page Style) ── */}
        <div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-slate-950 via-[#0A1E38] to-slate-900 text-white p-6 sm:p-8 shadow-xl border border-white/10">
          {/* Subtle Ambient Decorative Lighting Glows matching Home Hero */}
          <div className="absolute -top-24 -left-24 w-80 h-80 bg-[#0056B3]/25 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute -bottom-24 -right-24 w-80 h-80 bg-[#17A2B8]/20 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              ...
              <div className="relative shrink-0">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-600 text-slate-950 font-bold text-2xl flex items-center justify-center shadow-lg shadow-emerald-500/20 ring-4 ring-white/10">
                  {displayInitial}
                </div>
                <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 border-2 border-slate-950 flex items-center justify-center text-white" title="Active Account">
                  <Check className="w-3 h-3 stroke-[3]" />
                </div>
              </div>

              {/* Name, Verified Status & Metadata */}
              <div className="space-y-1.5">
                <div className="flex flex-wrap items-center gap-2.5">
                  <h1 className="text-2xl sm:text-3xl font-bold font-heading text-white tracking-tight">
                    Hello, {displayName}
                  </h1>
                  <span className="px-3 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-400/30 text-xs font-semibold backdrop-blur-md flex items-center gap-1.5 shadow-xs">
                ...
                <div className="text-xs text-slate-300 font-mono flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-slate-200 font-semibold">{formattedUserId}</span>
                  <span className="text-slate-600">•</span>
                  <span className="text-slate-400">Registered:</span>
                  <span className="text-slate-200 font-semibold">{formatRegistrationDate(user.created_at)}</span>
                </div>
              </div>
            </div>

            {/* Right Action: Edit Profile Glass Pill */}
            <div className="flex items-center gap-3 self-start lg:self-center">
              <button
                type="button"
                onClick={handleOpenEditModal}
                className="px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold flex items-center gap-2 transition-all backdrop-blur-md shadow-sm hover:scale-[1.02] cursor-pointer"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-white/10 text-center sm:text-left">
            ...
            <div className="bg-white/5 rounded-2xl p-3 border border-white/5 backdrop-blur-xs">
              ...
              <div className="text-xl font-bold font-mono text-white mt-0.5">
                {historyData?.active_medications?.length ?? 3}
              </div>
            </div>
            <div className="bg-white/5 rounded-2xl p-3 border border-white/5 backdrop-blur-xs">
              <div className="text-[11px] text-slate-400 font-medium flex items-center gap-1.5">
              ...
              <div className="text-xs font-bold text-emerald-300 mt-1 flex items-center gap-1">
                {isAbhaLinked ? "Linked & Active" : "Ready to Connect"}
              </div>
            </div>
          </div>
        </div>

        {/* ── 2. Three Interactive Action Cards ── */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {/* Card 1: Start New Intake */}
          <Link
            href="/kiosk"
            className="group relative overflow-hidden rounded-[26px] bg-white border border-emerald-200/80 hover:border-emerald-400 p-6 sm:p-7 shadow-xs hover:shadow-lg hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between space-y-5 cursor-pointer"
          >
            ...
            <div className="flex items-start justify-between">
              <div className="w-12 h-12 rounded-2xl bg-[#DCFCE7] text-[#16A34A] flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform">
            ...
            <div className="space-y-1.5">
              <h3 className="text-lg font-bold font-heading text-slate-900 group-hover:text-emerald-800 transition-colors">
                Start New Intake
              </h3>
            ...
            <div className="pt-2 flex items-center justify-between text-xs font-bold text-[#16A34A]">
              ...
              <span className="w-8 h-8 rounded-full bg-emerald-50 group-hover:bg-[#16A34A] group-hover:text-white flex items-center justify-center transition-all">
          ...
          <Link
            ...
            className="group relative overflow-hidden rounded-[26px] bg-white border border-blue-200/80 hover:border-blue-400 p-6 sm:p-7 shadow-xs hover:shadow-lg hover:shadow-blue-500/10 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between space-y-5 cursor-pointer"
          >
            ...
            <div className="flex items-start justify-between">
              <div className="w-12 h-12 rounded-2xl bg-[#DBEAFE] text-[#2563EB] flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform">
                <Clock className="w-6 h-6" />
              </div>
              <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-wider border border-blue-200">
                Longitudinal
              </span>
            </div>
            ...
            <div className="pt-2 flex items-center justify-between text-xs font-bold text-[#2563EB]">
              ...
              <span className="w-8 h-8 rounded-full bg-blue-50 group-hover:bg-[#2563EB] group-hover:text-white flex items-center justify-center transition-all">
                <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </span>
            </div>
          </Link>
          ...
          <Link
            href="/documents"
            className="group relative overflow-hidden rounded-[26px] bg-white border border-amber-200/80 hover:border-amber-400 p-6 sm:p-7 shadow-xs hover:shadow-lg hover:shadow-amber-500/10 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between space-y-5 cursor-pointer"
          >
            ...
            <div className="flex items-start justify-between">
              <div className="w-12 h-12 rounded-2xl bg-[#FFEDD5] text-[#EA580C] flex items-center justify-center shadow-xs group-hover:scale-105 transition-transform">
                <FileText className="w-6 h-6" />
              </div>
              <span className="px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 text-[10px] font-bold uppercase tracking-wider border border-amber-200">
                OCR Studio
              </span>
            </div>

            <div className="space-y-1.5">
              <h3 className="text-lg font-bold font-heading text-slate-900 group-hover:text-amber-800 transition-colors">
                Upload Documents
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed">
            ...
            <div className="pt-2 flex items-center justify-between text-xs font-bold text-[#EA580C]">
              <span>Upload Scans</span>
              <span className="w-8 h-8 rounded-full bg-amber-50 group-hover:bg-[#EA580C] group-hover:text-white flex items-center justify-center transition-all">
                <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </span>
            </div>
          </Link>
        ...
        {/* ── 3. Middle Section: Healthcare Identity (ABHA) & Recent Activity ── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Left Card: Healthcare Identity (ABHA) */}
          <div className="rounded-[26px] bg-white border border-slate-200/80 p-6 sm:p-7 shadow-xs flex flex-col justify-between space-y-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              ...
              {isAbhaLinked ? (
                <span className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-semibold flex items-center gap-1.5 shadow-2xs">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  Linked &amp; Verified
                </span>
              ) : (
                <span className="px-3 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200 text-xs font-semibold flex items-center gap-1.5 shadow-2xs">
            ...
            {/* ABHA Feature Graphic Box */}
            <div className="p-4 rounded-2xl bg-gradient-to-r from-slate-50 to-blue-50/50 border border-slate-200/70 flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-blue-600 shadow-xs shrink-0">
            ...
            <div className="space-y-3">
              <Link
                ...
                className="w-full py-3 rounded-full bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs flex items-center justify-center gap-2 transition-all shadow-sm hover:shadow-md cursor-pointer"
              >
          ...
          <div className="rounded-[26px] bg-white border border-slate-200/80 p-6 sm:p-7 shadow-xs flex flex-col justify-between space-y-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-slate-100 text-slate-700">
            ...
            {/* Timeline Activities List with Connected Line */}
            <div className="space-y-3.5">
              {recentActivities.map((act) => {
                const IconComponent = act.icon;
                return (
                  <div key={act.id} className="p-3 rounded-2xl bg-slate-50/70 border border-slate-100 flex items-center justify-between gap-3 hover:bg-slate-100/70 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-9 h-9 rounded-xl ${act.iconBg} ${act.iconColor} flex items-center justify-center shrink-0 shadow-2xs`}>
                    ...
                    <span className="text-[11px] text-slate-400 font-mono font-medium whitespace-nowrap shrink-0">
                      {act.timestamp}
                    </span>
                  </div>
                );
        ...
        <div className="rounded-[26px] bg-white border border-slate-200/80 p-6 sm:p-8 shadow-xs space-y-6 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            ...
            <button
              type="button"
              onClick={handleOpenEditModal}
              className="px-4 py-1.5 rounded-full border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs"
            >
        ...
        {/* ── Interactive Demographic Profile Edit Modal ── */}
        {editingProfile && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
            <div className="w-full max-w-lg bg-white rounded-[28px] p-6 sm:p-8 space-y-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">Update your verified personal patient information</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all cursor-pointer"
                    >
              ...
              <div className="flex gap-3 pt-3 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 py-3 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 shadow-sm hover:shadow-emerald-500/20"
                >
                  {savingProfile ? (
                    <>
                ...
                <button
                  type="button"
                  onClick={() => setEditingProfile(false)}
                  className="px-6 py-3 rounded-full border border-slate-300 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
              </div>
