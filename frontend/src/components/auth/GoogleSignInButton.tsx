...
export function GoogleSignInButton({ onSuccess, disabled }: GoogleSignInButtonProps) {
  ...
  const handleClick = async () => {
    ...
    setLoading(true);
    try {
      // In development/demo, if Google client ID is not configured, supply mock credential
      const googleClientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
      if (!googleClientId || googleClientId.includes("your-google")) {
        // Trigger simulated verified token for seamless local demo
        await onSuccess("mock_google_token_sample");
      } else {
        // Standard Google Identity Services prompt (if GIS is loaded)
        if (typeof window !== "undefined" && (window as any).google?.accounts?.id) {
          (window as any).google.accounts.id.prompt();
        } else {
          // Fallback to sample credential for demonstration
          await onSuccess("mock_google_token_sample");
        }
      }
    } catch (err) {
